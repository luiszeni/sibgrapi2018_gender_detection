...
float train_network(network *net, data d)
{
    ...
    int rows = d.X.rows;
    int batchsss = net->batch;

    //Verify if net batch size is equal to the training data batch size
    ...
    for(i = 0; i < n; ++i){
                     //data d, int n, int offset, float *X,   float *y
...
int resize_network(network *net, int w, int h)
{
    ...
    for (i = 0; i < net->n; ++i){
        ...
        }else if(l.type == REGION_ATTR){
            resize_region_attr_layer(&l, w, h);
...
void update_network_gpu(network *netp)
{
    ...
    ///for each layer updates the weights ?
    for(i = 0; i < net.n; ++i){
        layer l = net.layers[i];
        if(l.update_gpu){
            l.update_gpu(l, a);
