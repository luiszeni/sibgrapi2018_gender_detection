...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[region_attr]")==0) return REGION_ATTR;
...
layer parse_region_attr(list *options, size_params params)
{
    int coords = option_find_int(options, "coords", 4);
    int classes = option_find_int(options, "classes", 20);
    int attributes = option_find_int(options, "attributes", 0);
    int landmarks = option_find_int(options, "landmarks", 0);
    int max_boxes = option_find_int_quiet(options, "max",30);
    int num = option_find_int(options, "num", 1);

    layer l = make_region_attr_layer(params.batch, params.w, params.h, num, classes, coords, attributes, landmarks, max_boxes);
    assert(l.outputs == params.inputs);
...
network *parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        }else if(lt == REGION_ATTR){
            l = parse_region_attr(options, params);
